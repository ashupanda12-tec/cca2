...
All DynamoDB operations for the music subscription application.
...
Query logic (all filters are AND-combined per the spec):
  ...
  3. only album and/or year (no artist, no title)
                      → Scan with FilterExpression.
                        Scan is appropriate here because there is no viable
                        index entry point — the spec requires this case to work.
"""
...
def create_user(email: str, user_name: str, password: str) -> None:
    ...
    Raises ClientError if a conditional check fails (email already exists).
    """
...
def _build_filter_expr(conditions: list):
    """
    AND-combine a list of Attr conditions into a single FilterExpression.
...
def query_music(
    ...
    At least one filter must be non-empty (validated at the route layer).
    ...
    if title:
        ...
        if year:
            # Use LSI: title-year-index (title PK + year SK)
            key_cond = key_cond & Key("year").eq(year)
            kwargs = {
            ...
            # Query directly on main table PK
...
def get_subscriptions(email: str) -> list[dict]:
    ...
    Uses a Query on the partition key (email) — O(subscriptions), not a Scan.
    """
...
def add_subscription(
    ...
    Uses put_item — idempotent if the user subscribes to the same song twice.
    """
